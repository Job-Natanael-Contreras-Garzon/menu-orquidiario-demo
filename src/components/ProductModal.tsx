        ...
        <ScrollArea className="max-h-[90vh] rounded-lg">
          ...
          <div className="p-6 pt-4 space-y-4">
            <div className="flex justify-between items-start gap-4">
              <h2 className="text-3xl font-headline tracking-tight text-card-foreground pt-8">{t(item.name)}</h2>
              <p className="text-2xl font-bold text-amber-800 dark:text-amber-400 whitespace-nowrap pt-8">Bs. {item.price.toFixed(2)}</p>
            </div>
            ...
            {hasVariants && (
              <div>
                <h3 className="font-semibold mb-2 text-lg text-card-foreground">{t('productModal.titleVariants')}</h3>
                <div className="">
                  {item.variants?.map((variant: MenuItemVariant, index: number) => (
                    <div key={index} className="flex justify-between items-center bg-muted p-3 rounded-md">
                      <span className="text-sm font-semibold text-card-foreground">{t(variant.name)}</span>
                      <span className="text-sm font-semibold text-amber-800 dark:text-amber-400">+{variant.price.toFixed(2)} Bs.</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {item.ingredients && item.ingredients.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2 text-lg text-card-foreground">{t('productModal.titleIngredients')}</h3>
                <ul className="list-disc list-inside font-body text-muted-foreground space-y-1.5">
