...
export function ProductModal({ isOpen, onClose, item }: ProductModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md p-0 gap-0 overflow-hidden border-none shadow-2xl rounded-lg bg-[#FDF9F4]">
        ...
        <ScrollArea className="max-h-[90vh]">
          <div className="relative">
            ...
            {/* Back Button */}
            <Button variant="ghost" className="absolute top-4 left-4 h-auto bg-black/60 text-white rounded-full z-10 px-3 py-1.5 flex items-center gap-1.5 text-sm shadow-md transition-colors hover:bg-black/80" onClick={onClose}>
              <ArrowLeft size={16} />
          ...
          {/* Content Section */}
          <div className="p-6 space-y-4 text-[#4A372D]">
            <div className="flex justify-between items-start gap-4">
              <h2 className="text-3xl font-serif tracking-tight text-stone-800">{t(item.name)}</h2>
              <p className="text-2xl font-bold text-amber-800 whitespace-nowrap">Bs. {item.price.toFixed(2)}</p>
            </div>
            ...
            {hasVariants && (
              <div>
                <h3 className="font-semibold mb-2 text-lg text-stone-800">{t('productModal.variants')}</h3>
                <div className="space-y-2">
                  {item.variants?.map((variant: MenuItemVariant, index: number) => (
                    <div key={index} className="flex justify-between items-center bg-stone-50 p-3 rounded-md">
                      <span className="text-sm font-medium text-stone-700">{t(variant.name)}</span>
            ...
            {item.ingredients && item.ingredients.length > 0 && (
              <div>
                <h3 className="font-semibold mb-2 text-lg text-stone-800">{t('productModal.ingredients')}</h3>
                <ul className="list-disc list-inside font-body text-stone-600 text-sm space-y-1.5">
